        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <Select value={customerId} onValueChange={setCustomerId}>
              ...
              <SelectContent>
                {customers.map(c => (
                  <SelectItem key={c.id} value={c.id}>{c.type === 'village' ? '🏡' : '🏙️'} {c.name}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
          ...
          <Button type="submit" className="w-full" disabled={loading}>{loading ? 'Saving...' : 'Save Entry'}</Button>
        </form>
      </DialogContent>
    </Dialog>
  );
